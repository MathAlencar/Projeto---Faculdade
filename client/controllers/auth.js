...
exports.register = (req, res) => {
        ...
        // return res.render('08.cadastrarFuncionarios.hbs');
        ...
        mysql.query(`SELECT * FROM tbl_User WHERE email_Login = ?`, [email], async (err, result) => {
            ...
            if(result.length > 0) return res.render('08.cadastrarFuncionarios.hbs', {
                message: "Este e-mail já está cadastrado"
                // res.render() --> Significa que eu estou recarregando a página html/hbs
            })
    
            // Verificando se ambas as senhas são iguais;
            else if( senha !== confirmarSenha) return res.render('08.cadastrarFuncionarios.hbs', {
            ...
            mysql.query('INSERT INTO tbl_User (nome, email_Login, password_Login, telefone) VALUES (?,?,?,?);  ', [nome+' '+sobrenome, email, hashPassword, number], (err, results) => {
                    if(err) {
                        console.log(err);
                    } else {
                        console.log(results)
...
exports.loginUser = (req, res) => {
    ...
    mysql.query(query, [email], (err, results) => {
        ...
        bcrypt.compare(senha, results[0].password_Login, (err, result) => {
            ...
            return res.render('101.mobile.login.hbs', console.log('deu erro menorasas'))
        })
    } )  
}
